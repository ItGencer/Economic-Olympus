...
as $$
  with cells(id, ring, cell_type, params) as (
    values
      ...
      ('inner-vacation-04', 'inner', 'image', '{"label":"Image","priceMin":100,"priceMax":3000,"imageMin":1,"imageMax":10}'::jsonb),
      ('inner-image-06', 'inner', 'image', '{"label":"Image","priceMin":100,"priceMax":3000,"imageMin":1,"imageMax":10}'::jsonb),
      ('inner-negative-reputation-07', 'inner', 'image', '{"label":"Image","priceMin":100,"priceMax":3000,"imageMin":1,"imageMax":10}'::jsonb),
      ('inner-salary-08', 'inner', 'salary', '{"label":"Salary","dieMin":1,"dieMax":20,"bonusUnit":1000,"fineUnit":100}'::jsonb),
      ...
      ('inner-negative-reputation-15', 'inner', 'negative_reputation', '{"label":"Negative reputation","dieMin":1,"dieMax":6}'::jsonb),
      ('inner-salary-16', 'inner', 'salary', '{"label":"Salary","dieMin":1,"dieMax":20,"bonusUnit":1000,"fineUnit":100}'::jsonb),
      ...
      ('outer-negative-reputation-09', 'outer', 'negative_reputation', '{"label":"Negative reputation","dieMin":1,"dieMax":6}'::jsonb),
      ('outer-salary-12', 'outer', 'salary', '{"label":"Salary","dieMin":1,"dieMax":20,"bonusUnit":1000,"fineUnit":100}'::jsonb),
      ...
      ('outer-salary-32', 'outer', 'salary', '{"label":"Salary","dieMin":1,"dieMax":20,"bonusUnit":1000,"fineUnit":100}'::jsonb)
  )
...
create or replace function public.is_salary_cell(p_cell_id text)
returns boolean
language sql
immutable
security definer
set search_path = public
as $$
  select coalesce(public.get_board_cell_config(p_cell_id)->>'type', '') = 'salary';
$$;

grant execute on function public.is_salary_cell(text) to authenticated;

create or replace function public.create_salary_pending_action(
  p_game_id uuid,
  p_player_id uuid
)
returns jsonb
...
begin
  ...
  if not public.is_salary_cell(v_cell_id) then
    raise exception 'not_salary_cell';
  end if;
  ...
  v_kind := case when v_image > 0 then 'bonus' when v_image < 0 then 'fine' else 'neutral' end;
  v_unit := case when v_image > 0 then 1000 when v_image < 0 then 100 else 0 end;
  ...
  return jsonb_build_object(
    ...
    'event_type', 'cell_salary_pending',
...
grant execute on function public.create_salary_pending_action(uuid, uuid) to authenticated;

create or replace function public.resolve_salary(
  p_game_id uuid,
  p_decision text
)
returns jsonb
...
begin
  ...
  if v_decision not in ('roll', 'confirm') then
    raise exception 'invalid_salary_decision';
  end if;
  ...
  if v_pending_action is null
    ...
    or v_pending_action->>'type' <> 'salary' then
    raise exception 'salary_pending_action_required';
  end if;
  ...
  if v_decision = 'roll' then
    if v_phase <> 'initial' then
      raise exception 'salary_roll_already_resolved';
    end if;

    v_die := public.random_int_between(1, 20);
    v_amount := v_die * v_unit;
    v_balance_delta := case when v_kind = 'bonus' then v_amount when v_kind = 'fine' then -v_amount else 0 end;
  ...
  if v_phase <> 'roll_ready' then
    raise exception 'salary_roll_required';
  end if;

  v_die := coalesce(nullif(v_payload->>'die', '')::integer, 0);

  if v_die < 1 or v_die > 20 then
    raise exception 'salary_roll_required';
  end if;
...
grant execute on function public.resolve_salary(uuid, text) to authenticated;
